...
func SetupGroupRoutes(appRouterGroup *AppRouterGroup, h *di.HandlerProvider) {
	...
	appRouterGroup.AuthRouterGroup.GET("/group/info/:group_id", h.GroupHandler.GetGroupInfo)
	appRouterGroup.AuthRouterGroup.GET("/group/list", h.GroupHandler.GetGroupList)
	appRouterGroup.AuthRouterGroup.POST("/group/add_member/:group_id", h.GroupHandler.AddGroupMembers)
